...
class Sine():
    def __init__(self, npts=100, domain=(-5, 5)):
        self.npts = npts
        self.domain = domain  # this is the domain on which function is defined.
        self.f0 = 1
        self.amp = np.random.uniform(1.0, 5.0)
        self.offset = 0
        self.ph0 = np.random.uniform(0, np.pi)

    def __str__(self):
        return "f0={self.f0:0.2f}, amp={self.amp:0.2f}".format(self=self)

    def fn(self, xs):
        return np.sin(self.f0 * xs + self.ph0) * self.amp
    ...
    def samples(self, n, domain=None):
        """Generates n samples (uniformly) in domain.
        :param n: number of samples to generate
        :param domain: this is the domain on which to sample
        :return: xs, ys
        """
        # xs = np.sort(np.random.uniform(*(domain or self.domain), n))  # return sorted so that it is easier to use
        # ys = self.fn(xs)
        xs, ys = self.proper()
